...
- **实时**：Server-Sent Events 推流，回复逐字显示，含 💭 思考过程、🔎 联网搜索、⚙ 命令执行，且出现在真实发生的位置。
- **同账号**：复用 `~/.codex/` 下的 ChatGPT 登录态，会话线程写入 Codex 的线程数据库（与桌面 App 同源；App 只在启动时读取，需重启 App 才能看到新会话）。
...
- Python 3.10+
- 已登录的 [Codex CLI](https://developers.openai.com/codex/cli/)。`codex app-server` 需在 `~/codex-bin/codex.exe` 或 PATH 上，可用环境变量 `CODEX_BIN` 指定。
...
也可以作为编程 Agent（ZCode / Claude Code 等）的配套工具：Agent 通过 `POST /api/send` 直接向会话发消息，用户在网页旁观实时输出。参考 `skill/` 目录下的示例 skill。
...
| `/api/new` | POST `{project?, model?, sandbox?}` | 新会话；sandbox: read-only / workspace-write / danger-full-access |
