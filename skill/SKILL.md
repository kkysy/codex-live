...
**Agent 低成本监控（省 token）**：
- `GET /api/last_text` — 只返回最后一条 assistant 消息的**正文 text 段** + 命令/思考段计数。勿用 `/api/state` 做轮询：其 parts 含全部命令调用文本（单轮可达 3 万字符），全量拉取浪费 token。
...
**用户想让 Agent 知道 ChatGPT 说了什么** → `curl -s http://127.0.0.1:8765/api/state`，`messages` 数组含全部历史（role: user/assistant）。
